                ...
                <form action="{{ route('notes.update', $note->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('class_id') is-invalid @enderror" id="class_id"
                            ...
                            <option value="{{ $class->id }}" {{ (old('class_id', $note->class_id) == $class->id) ?
                                'selected' : '' }}>
                                {{ $class->name }}
                            </option>
                            @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('content') is-invalid @enderror" id="content"
                            name="content" rows="6" required>{{ old('content', $note->content) }}</textarea>
                        @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        @if($note->file_path)
                        <div class="mb-2">
                            <strong>Current file:</strong> {{ $note->file_name }}
                            <a href="{{ route('notes.download', $note->id) }}" class="btn btn-sm btn-info ml-2">
