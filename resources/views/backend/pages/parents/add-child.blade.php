                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control @error('student_id') is-invalid @enderror"
                                        ...
                                        <option value="{{ $student->id }}" {{ old('student_id')==$student->id ?
                                            'selected' : '' }}
                                            data-name="{{ $student->name }}"
                                            data-roll="{{ $student->roll_number ?? 'Not assigned' }}"
                                            data-class="{{ $student->class ?? 'Not assigned' }}">
                                            {{ $student->name }} - Roll: {{ $student->roll_number ?? 'Not assigned' }}
                                            (Class:
...
<script type="text/javascript">
    ...
    function showStudentDetails(selectElement) {
        if (selectElement.value) {
            ...
            document.getElementById('selected_student_image').src = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(name) + '&background=random';
